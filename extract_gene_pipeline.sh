#!/bin/bash
: "${1:?Usage: $0 cnsg_script_dir <gene> <taxon> <feature_type> <limit>}"
...
command time -v uv run --project "$script_dir" "$script_dir"extract_gene.py "$taxon"/GENOMIC1/ "$taxon"/GFF1/ "$output_dir"/ "$genes" "$feature_type" "$max_len"
